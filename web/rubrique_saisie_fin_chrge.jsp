...
<%@page import="model.V_indicateur_fin_charge"%>
<%@page import="model.V_indicateur_fin_charge_prev"%>
            ...
            <div class="d-flex flex-column" id="content-wrapper">
                <div id="content">
                    ...
                    <div class="container-fluid">
                        <h3 style="text-align: center;margin-bottom: 2%;">Tableau de bord - Finances charges & produits</h3>
                        ...
                        <div style="display:flex;gap:45px;justify-content:flex-start;">
                            ...
                            <div><canvas id="finChargeChart" width="400" height="400"></canvas>
                                <p class="text-center">Produits et charges</p>
                            </div>
...
//                            V_tendance_fin[] tendances = gson.fromJson(json_tendance_fin, V_tendance_fin[].class);
                            //fin charge data 
                            String jsonFinCharge = request.getAttribute("fin_charge_data").toString();
                            String jsonFinChargePrev = request.getAttribute("fin_charge_prev_data").toString();

                            V_indicateur_fin_charge[] charges = gson.fromJson(jsonFinCharge, V_indicateur_fin_charge[].class);
                            V_indicateur_fin_charge_prev[] chargesPrev = gson.fromJson(jsonFinChargePrev, V_indicateur_fin_charge_prev[].class);
                        ...
                        <script>
                            let chargesStack = [];
                            let totalCourant = new Array(12).fill(0);
                            let totalPrev = new Array(12).fill(0);
                        </script>

                        <%
                            for (V_indicateur_fin_charge item : charges) {

                                String groupe = item.getGroupe_rubrique_lib();
                                String designation = item.getDesignation(); // ATTENTION : REA_N1
%>
                        ...
                        <form action="controller_updateVoletFIN" method="post">
                            ...
                            <div class="card bg-light mb-3">
                                <h5 class="card-header">Fin charge</h5>
                                <div class="card-body">
                                    <div class="row g-3">
                                        <div class="col-12" style='overflow-x:scroll;'>
                                            ...
                                            <table border="1" class="table-responsive table-bordered">
                                                ...
                                                <% for (V_indicateur_fin_charge item : charges) {
                                                        String anneeTendance = item.getAnnee().toString();
                                                %>
                                                <tr>
                                                    <td>
...
//            }
            ...
            try {
                ...
                function buildChart(selectedMonth) {
                    ...
                    tendancesData.forEach(item => {
                        ...
                        if (item.annee == <%= annee - 2%>)
                            index = 0;
                        else if (item.annee == <%= annee - 1%>)
                            index = 1;
                        else if (item.annee == <%= annee%>)
                            index = 2;
        ...
        <script>
            try {
                ...
                // 🔥 datasets final
                let datasets = [...chargesStack];

                // 👉 ligne TOTAL (année en cours)
                datasets.push({
                    type: 'line',
                    label: 'Total',
                    data: totalCourant,
                ...
                // 🔥 chart
                const ctx = document.getElementById('finChargeChart').getContext('2d');
                ...
                console.log('Error charte charges fin: ' + error);
            }
        </script>
    </body>
</html>
